...
<p align="center">
  <img src="../Images/stark-scanning.gif" width="700">
</p>
...
This is where StarkOS stops reacting blindly and starts observing.
...
Cameras are the robot’s primary visual interface with the world.
In simulation, this lets us validate perception pipelines, image transport, and data flow
...
On a real mobile robot, camera streams are almost always transmitted over
...
<p align="center">
  <img src="../Images/iron-monger-overload.gif" width="700">
</p>
...
Robotics systems rarely run with fixed parameters.
...
<p align="center">
  <img src="../Images/iron-man-lidar.gif" width="700">
</p>
...
Unlike cameras, LiDAR does not interpret the world — it measures it.
...
At this point, the robot can move, see, and measure its surroundings.
More importantly, you now understand how sensor data flows
from simulation to ROS — exactly as it would on real hardware.

StarkOS now has perception.
...
![alt text](../Images/onemoreAssignment.gif)

### One More Assignment 😌

Yeah… we know.  
That look? That’s exactly how Tony Stark feels when FRIDAY says *“one more test run.”*

But hear us out — this one’s actually **fun**.

You’re not just wiring things up anymore.  
Your robot can **see**, **sense**, and now it’s time to let it **think a little**.

This assignment is about giving your Iron Rover a bit of autonomy —  
...
- move without teleoperation

Think of this as Stark switching from *manual test mode* to *autonomous assist*.
...
assgn
```
Inside the package, create the following folder structure:
```bash
assgn/
├── launch/
├── rviz/
...
Place the downloaded files into your package folders as follows:

- Copy **`Assgn.launch.py`** and **`world.launch.py`** into the `launch/` folder.

- Copy **`rviz.rviz`** into the `rviz/` folder.

- Copy **`erc_bot.gazebo`**, **`IronRover.urdf`**, and **`materials.xacro`** into the `models/` folder.

- Copy the mesh files **`IronMan_ROS_Body.dae`**, **`IronManWheelLeftFront.dae`**, **`IronManWheelRightFront.dae`**, and **`lidar.dae`** into the `meshes/` folder.

- Copy **`IronManCave.sdf`** into the `worlds/` folder.

### Autonomous Logic Node
Inside the `assgn/` folder create the following file:
```bash
tonystarklogic.py
```
...
- process obstacle distances
- publish cmd_vel
- enable autonomous exploration and obstacle avoidance

You do not need to write everything from scratch.

A template file is provided with missing parts.
Read the code carefully, understand what each block does, and fill in the blanks.

after making the tonystarklogic.py

run command

```bash
chmod +x tonystarklogic.py 
```
So it can become a executable

### 🚀 Running the Assignment
Launch the simulation
```bash
ros2 launch assgn Assgn.launch.py
```
and then run your logic
```bash
ros2 run assgn tonystarklogic
```

![alt text](../Images/tonybestofluck.gif)
